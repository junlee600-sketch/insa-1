...
export default function EvaluationForm() {
  ...
  const fetchData = async () => {
    try {
      ...
      const assnDoc = await getDoc(doc(db, 'assignments', assignmentId));
      ...
      if (assn.status === 'completed') {
        setIsCompleted(true);
        const resultDoc = await getDoc(doc(db, 'results', assignmentId));
        if (resultDoc.exists()) {
  ...
  const executeSubmit = async () => {
    ...
    try {
      ...
      const resultRef = doc(db, 'results', assignmentId);
