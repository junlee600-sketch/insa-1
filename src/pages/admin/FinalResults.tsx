  ...
  return (
    <div className="space-y-6">
      ...
      {selectedYear && (
        <div className="flex-1 border border-[#1A1A1A] overflow-hidden flex flex-col">
          <div className="grid grid-cols-12 bg-[#1A1A1A] text-white text-[10px] uppercase tracking-[0.15em] p-4 sticky top-0">
            <div className="col-span-3">대상자 이름</div>
            <div className="col-span-2 text-center">진행률</div>
            <div className="col-span-2 text-center">원점수 평균</div>
            <div className="col-span-2 text-center">최종 확정 점수</div>
              ...
              filteredEvaluatees.map(ev => {
                ...
                return (
                  <div key={ev.evaluateeId} className="grid grid-cols-12 p-4 border-b border-[#EEE] items-center hover:bg-[#F9F9F9] cursor-default group">
                    <div className="col-span-3 font-bold">{usersMap[ev.evaluateeId] || ev.evaluateeId}</div>
                    <div className="col-span-2 font-sans text-xs uppercase text-[#777] text-center">
                      <span className={isComplete ? 'text-emerald-700' : 'text-amber-700'}>
                         {ev.totalCompleted} / {ev.totalAssigned}
                      </span>
                    </div>
                    <div className="col-span-2 text-center font-sans text-xs bg-[#F0F0F0] py-1 mx-4">
                      {rawAvg}
                    </div>
                    ...
                    <div className="col-span-2 text-center">
                      {ev.finalState ? (
                        <span className="text-[9px] uppercase tracking-widest px-2 py-1 bg-[#1A1A1A] text-white">확정됨</span>
                      ) : (
                        <span className="text-[9px] uppercase tracking-widest px-2 py-1 bg-[#E8F5E9] text-emerald-800 border border-emerald-100">대기 중</span>
                      )}
                    </div>
      ...
      <Dialog open={modalOpen} onOpenChange={setModalOpen}>
        <DialogContent className="max-w-3xl border-[#1A1A1A] rounded-none bg-[#FDFDFB] p-0">
          <DialogHeader className="p-8 border-b border-[#E5E5E5] bg-[#F9F9F9]">
            <DialogTitle className="text-3xl font-normal leading-none text-[#1A1A1A]">
              최종 점수 확정
              <span className="block mt-2 font-sans font-bold text-lg text-[#555] tracking-tight">{usersMap[selectedEvaluatee?.evaluateeId]}</span>
            </DialogTitle>
          </DialogHeader>
                ...
                {selectedEvaluatee?.assignments.map((assn: any) => (
                  <div key={assn.id} className="p-3 border-b border-[#EEE] hover:bg-[#F9F9F9] transition-colors">
                    <div className="grid grid-cols-12 items-center">
                      <div className="col-span-4 font-bold">{usersMap[assn.evaluatorId] || assn.evaluatorId}</div>
