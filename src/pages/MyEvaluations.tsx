...
export default function MyEvaluations() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      usersSnap.docs.forEach(d => { umap[d.data().email] = d.data().name; });
      setUsersMap(umap);
      ...
      {activeYear && (
        <div className="flex-1 border border-[#1A1A1A] overflow-hidden flex flex-col">
          <div className="grid grid-cols-12 bg-[#1A1A1A] text-white text-[10px] uppercase tracking-[0.15em] p-4 sticky top-0">
            <div className="col-span-3">평가 대상자</div>
            <div className="col-span-3">대상자 그룹</div>
            <div className="col-span-3">진행 상태</div>
            ...
            {assignments.length === 0 ? (
              ...
              assignments.map(a => (
                <div key={a.id} className="grid grid-cols-12 p-4 border-b border-[#EEE] items-center hover:bg-[#F9F9F9] transition-colors">
                  <div className="col-span-3 font-bold">{usersMap[a.evaluateeId] || a.evaluateeId}</div>
                  <div className="col-span-3 font-sans text-xs uppercase text-[#777]">{groupsMap[a.groupId] || a.groupId}</div>
